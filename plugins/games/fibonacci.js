export const run = {
   ...
   async: async (m, {
      ...
      const isO = Utils.random(['+', '*'])
      const isX = Utils.randomInt(1, isO == '*' ? 3 : 25)
      const isY = Utils.randomInt(isO == '*' ? 4 : 26, isO == '*' ? 7 : 50)
      const isF = Utils.randomInt(1, isO == '*' ? 5 : 7)
      const isR = Utils.randomInt(1, isF)
      const isD = Utils.fibonacci(isX, isY, isF, isO)
      const isS = isD[isR]
