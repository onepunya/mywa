export const run = {
   usage: ['brainwhat', 'clue', 'flagclue', 'picclue', 'quizclue', 'songclue', 'wordclue', 'who', 'cakclue', 'jktclue'],
   async: async (m, {
      client,
      command,
      Utils
         ...
         if ((id in client.quiz)) {
            ...
            for (let i = 0; i < json.jawaban.length; i++) {
               if (client.quiz[m.chat][3].includes(json.jawaban[i])) {
                  ...
                  clue += '```' + Utils.ucword(json.jawaban[i].replace(/[bcdfghjklmnpqrstvwxyz]/g, '_')) + '```\n'
               }
            }
            let pop = clue.split('\n')
            pop.pop()
      ...
      } else if (command == 'jktclue') {
         client.jkt = client.jkt ? client.jkt : {}
         if ((id in client.jkt)) {
            const clue = client.jkt[id][1].name.replace(/[bcdfghjklmnpqrstvwxyz]/g, '_')
            client.reply(m.chat, '🚩 Clue : ```' + clue + '```', m)
         }
      }
   },
